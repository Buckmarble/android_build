...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^DS_") ; then
       DS_BUILD=$(echo -n $1 | sed -e 's/^DS_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $DS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       DS_BUILD=
    fi
    export DS_BUILD
...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$DS_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $DS_BUILD, run away!"
    fi
}
